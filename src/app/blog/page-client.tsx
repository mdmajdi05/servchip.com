...
export default function BlogPage() {
  ...
  const isSearching =
    searchQuery !== debouncedSearch && searchQuery.length >= 1;
          ...
          (s) =>
            ...
            (s.content || []).some((c) => {
              ...
              if (c.type === "bulletList" || c.type === "numberedList")
                return c.items.some((i) => i.toLowerCase().includes(q));
              if (c.type === "faq")
                return c.items.some(
                  (f) =>
  ...
  const showNoResults =
    !isSearching && debouncedSearch.length >= 2 && resultCount === 0;
        ...
        {showNoResults && (
          <div className="text-center mt-16">
            ...
            <p className="text-text-dim text-sm">
              Try a different search term, remove filters, or browse all
              categories.
            </p>
          </div>
        )}
